...
var localStore = scload("plugins/scriptcraft/data/drone-store.json") || {}
var droneStore = plugin('drone', {store: localStore }, true)
global.droneStore = droneStore
...
var _onPlayerInteractEvent = function (listener, event){    
    //var localStore = scload("plugins/scriptcraft/data/drone-store.json")
    //localStore = localStore || {}
    var loc = event.getClickedBlock().getLocation().getWorld().getName() + "@" + event.getClickedBlock().getX() + "@" + event.getClickedBlock().getY() + "@"+ event.getClickedBlock().getZ()
        try {
            if ((typeof localStore)  == 'undefined') return;

            if (localStore[loc])
                event.player.sendMessage("This block was created with function: "+localStore[loc].func);
        } 
    catch (e) {
    }
}
events.on( 'player.PlayerInteractEvent', _onPlayerInteractEvent, "LOWEST")
...
var Drone = function( x, y, z, dir, world ) {
    ...
    var not_op = typeof(self) != "undefined" && !self.op

        if((not_op) && !(y instanceof bkLocation && x instanceof bkPlayer))
        {
            throw new Error("You must supply a location and a player to the drone object")
        } else if (not_op){
            playerPos = y;
            player = x;
        } else{
            if(x instanceof bkLocation){
                playerPos = x;
            }else{
                if(y instanceof bkPlayer)
                    player = y
...
Drone.tick = setInterval( function() {
    ...
    while ( maxOpsPerTick > 0 ) { 
        ...
        if(org.bukkit.Material.getMaterial(op.typeid) == null)
        {
            bad = true
        }
        ...
        if(!bad){
            ...
            block.data = op.meta;
            server.getPluginManager().getPlugin("ThoughtStem").getBlockOrientationClass().orientProperly(block);
            try { 
                if (op.player.getMetadata("func").size() > 0) {
                    var loc = block.getLocation().getWorld().getName() + "@" + 
                        block.getX() + "@" + block.getY() + "@"+block.getZ()    
                        var pname = "" + op.player.getName()
                        var funcname = "" + op.player.getMetadata("func").get(0).asString()
                        if(!droneStore.store[loc])
                          droneStore.store[loc] = {}

                        droneStore.store[loc].name = pname
                        droneStore.store[loc].func = funcname
                } 
            }
            catch (e) {
            ...
            //broadcast(droneStore.store)
            //block.setMetadata("test", new org.bukkit.metadata.FixedMetadataValue(__plugin, op.player.getName())) 
            //block.setMetadata("func", op.player.getMetadata("func").get(0))
        }
...
Drone.prototype.mob = function ( monster ) {
    if (typeof monster == 'undefined' ) { //if they used no params
        monster = "PIG"; //default to pig
    }
    var spawnLoc = new bkLocation(this.world, this.x, this.y, this.z);
    try {
...
Drone.prototype.cuboid = function( block, w, h, d ) {
    if(block instanceof org.bukkit.Material)
      block = block.getId()
...
Drone.extend('spawnmob', Drone.prototype.mob );
Drone.extend('setLocation', Drone.prototype.setLocation);

Drone.prototype.setLocation = function(loc) {
    this.x = Math.floor(loc.getX());
    this.y = Math.floor(loc.getY());
    this.z = Math.floor(loc.getZ());
    return this;
}
//
