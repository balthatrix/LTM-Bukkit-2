...
var _onPlayerCommand = function( listener, event ) {
    var message = event.getMessage();
    
    if (event.getMessage().indexOf("/reset") == 0) {
        event.player.sendMessage("Clearing your intervals and events...");
        clearIntervalsAndEvents(event.player.name); //clears intervals for player
        event.setCancelled(true); //stop any other further event from processing
        return; // break out
    }
    ...
    if(event.getMessage().indexOf("/js") == 0)
    {
        ...
        var newMessage = "/js try{" + message.substring(4) + "}" +
            "catch(e){ "+
            "if (e.message.indexOf('"+playerName+"') > 0 && e.message.indexOf('is not defined') > 0) {"+
            "self.sendMessage(org.bukkit.ChatColor.translateAlternateColorCodes('#', '#4Did you remember to load your script?'));" +
            "} else {" +
            "self.sendMessage(e.message);"+
            "self.sendMessage('On line #:'+e.lineNumber)}}";
        event.player.setMetadata("func", new org.bukkit.metadata.FixedMetadataValue(__plugin, message.substring(4)));
    ...
    if(result)
    {
		...
		/*
            if(event.getMessage().indexOf("/plotme home") == 0)
            {
                global.plots[event.player.getName()] = event.player.location
                    broadcast(event.player.getName() + "'s home is " + event.player.location)
            }
		*/



        if(score[event.player.getName()] == undefined)
            score[event.player.getName()] = 1 

                scoreboard.update( 'interactions', event.player, score[event.player.getName()]++);
    }
...
var interval = function(player, ref, time){
  if (player && global.intervals) {
      ...
      global.intervals[player.name].push(setInterval(ref,time))
  }
}
